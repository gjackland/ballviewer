...
/**
 * A histogram that can be built by the user and displayed if necessary.
 * <p>
 * <code>LightHistogram</code> is a class which has the same functionality,
 * except for the graphical component; it can't be displayed. The data in a
 * <code>LightHistogram</code> can be displayed by adding it using
 * {@link #addData(LightHistogram)}.
 * <p>
 * <code>RadialHistogram</code> and <code>AngularHistogram</code> are subclasses
 * of <code>Histogram</code> that each set an appropriate title, size and marks
 * for the type of histogram.
 */
public class Histogram extends Frame implements ActionListener
{
	...
	double			step;		// =(range/freq.length) ie the width of each
								// bar: same

	/**
	 * Creates a new histogram, with bars of width <code>stp</code>. It will
	 * cover the range <code>mn</code> to <code>mx</code> and have the name
	 * <code>n</code>.
	 */
	...
	{
	} // belongs to Actionlistener Interface
	...
	/**
	 * Returns the weighted sum of the bars over the given range, focussed on
	 * <code>peak</code>.
	 */
	public double weightedSum( double x1, double peak, double x2 )
	{
		int i1 = ( int )Math.floor( ( ( x1 - min ) / step ) + 0.5 );
		int ip = ( int )Math.floor( ( ( peak - min ) / step ) + 0.5 ); // weighted
																		// by a
																		// triangle
		int i2 = ( int )Math.floor( ( ( x2 - min ) / step ) + 0.5 );
		double ws = 0;
		for( int i = i1; i < ip; i++ )
			ws += freq[ i ] * ( i - i1 ) / ( double )( ip - i1 );
		for( int i = ip; i <= i2; i++ )
			ws += freq[ i ] * ( i2 - i ) / ( double )( i2 - ip );
		return ws;
	}

	/** Returns an approximate mean value of x over the given range. */
	public double mean( double x1, double x2 )
	{ // ie the mean x position of the data in this range
		int i1 = ( int )Math.floor( ( ( x1 - min ) / step ) + 0.5 );
		int i2 = ( int )Math.floor( ( ( x2 - min ) / step ) + 0.5 );
		double m = 0;
		for( int i = i1; i <= i2; i++ )
	...
	public void draw()
	{
		...
		{ // this code allows window to be closed //
			@Override
		...
		for( i = 0; i < freq.length; i++ )
		{
			...
			gr.fillRect( i * w, 0, w, ( int )( freq[ i ] * h ) ); // this draws
																	// the bar
			gr.setColor( Color.gray );
			for( j = 10; j < freq[ i ] * h; j += 10 )
				gr.drawLine( i * w, j, i * w + w, j ); // puts ticks on bar
		}
		for( i = 0; i < W; i += 10 )
			gr.drawLine( i, 0, i, 2 ); // the x axis tick
...
class BufferedCanvas extends Canvas
{
	...
	{ // this won't work until frame is displayed
		if( buffer != null )
			g.drawImage( buffer, 0, 0, this ); // so need to check it is
												// possible
	}
}
