...
public class Matrix3
{
	...
	public Matrix3( double r00, double r01, double r02, double r10, double r11, double r12, double r20, double r21, double r22 )
	{
		r[ 0 ][ 0 ] = r00;
		r[ 0 ][ 1 ] = r01;
		r[ 0 ][ 2 ] = r02;
	...
	{
		Vector3 f = fwd.normalised(); // forward is positive z (crucial for
										// camera)
	...
	/**
	 * Returns result of multiplying by <code>m</code> in front (
	 * <code>mult(m,this)</code>).
	 */
	public Matrix3 fmult( Matrix3 m )
	{
		return mult( m, this );
	} // multiply at front

	/**
	 * Returns result of multiplying by <code>m</code> behind (
	 * <code>mult(this,m)</code>).
	 */
	public Matrix3 bmult( Matrix3 m )
	{
		return mult( this, m );
	} // multiply at back
	...
	{
		return r[ 0 ][ 0 ] * ( r[ 1 ][ 1 ] * r[ 2 ][ 2 ] - r[ 1 ][ 2 ] * r[ 2 ][ 1 ] ) + r[ 0 ][ 1 ] * ( r[ 1 ][ 2 ] * r[ 2 ][ 0 ] - r[ 1 ][ 0 ] * r[ 2 ][ 2 ] ) + r[ 0 ][ 2 ]
				* ( r[ 1 ][ 0 ] * r[ 2 ][ 1 ] - r[ 1 ][ 1 ] * r[ 2 ][ 0 ] );
	}

	/** Routine for eliminating cumulative errors (probably unnecessary). */
	public void orthonormalise()
	{ // this routine should erase any cumulative errors
		Vector3 row1 = new Vector3( r[ 1 ][ 0 ], r[ 1 ][ 1 ], r[ 1 ][ 2 ] );
		Vector3 row2 = new Vector3( r[ 2 ][ 0 ], r[ 2 ][ 1 ], r[ 2 ][ 2 ] ); // keeps
																				// z
																				// direction
		row1.normalise();
		row2.normalise();

		Vector3 row0 = row1.cross( row2 ).normalised(); // ensures that row0
														// orthonormal to row1,
														// row2
		row1 = row2.cross( row0 ).normalised(); // ensures that row2 is also o/n
												// to row1

		r[ 0 ][ 0 ] = row0.x;
		r[ 0 ][ 1 ] = row0.y;
		r[ 0 ][ 2 ] = row0.z; // same as for Matrix3(v,v,v)
		r[ 1 ][ 0 ] = row1.x;
		r[ 1 ][ 1 ] = row1.y;
		r[ 1 ][ 2 ] = row1.z;
		r[ 2 ][ 0 ] = row2.x;
