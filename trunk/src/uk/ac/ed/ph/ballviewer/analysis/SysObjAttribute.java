...
public final class SysObjAttribute
{
	...
	public static ArrayList< SysObjAttribute >
	getAttributes( final Class		sysObjClass )
	{
		...
		for( Field f : sysObjClass.getFields() )
		{
			...
			if( fAnnot != null )
			{
				...
				if( attribDetails != null )
				{
					...
					{
						System.out.println( "Unable to set default value for attribute method " + attribDetails.setterMethod +
							". Ignoring..." );
					}
					catch( IllegalAccessException e )
					{
						System.out.println( "Unable to get default value for attribute " + fAnnot.name() +
							" check that it is declared public. Ignoring..." );
					}
				}
				else
				{
					System.out.println( "Class " + sysObjClass + " has attribute default value " + fAnnot.name() + 
						" but no corresponding annotated setter method.  Ignoring..." );
				}
			}
		}
		...
		for( java.util.Map.Entry< String, AttributeDetails > entry : attribDetailsMap.entrySet() )
		{
			try
			{
				...
				final SysObjAttribute newAttrib = new SysObjAttribute(
					attribDetails.setterMethod.getParameterTypes()[ 0 ],
					attribDetails.setterMethod,
					attribDetails.defaultValue,
					entry.getKey()
				);
				attributes.add( newAttrib );
				
			}
			catch( IllegalArgumentException e ){}	// This attribute won't work but we've already told the user the error previously so carry on
	...
	private final Class			sysObjClass;				// The class type of the system object that this attribute is from
	private final Method		setter;						// The setter method used to set the attribute
	private final Class			attributeClass;				// The parameter class passed to the setter
	private final Object		defaultValue;				// The default value for the attribute if not being set by analyser output
	...
	) throws IllegalArgumentException
	{
		...
		if( attributeClass == null || setter == null ||
			setterParamTypes.length != 1 ||
			!Lib.areClassesCompatible( setterParamTypes[ 0 ], defaultValue.getClass() ) )
		{
	...
	)
	{
		if( values.length != objArray.length ||
			values.length == 0 ||
			!Lib.areClassesCompatible( sysObjClass, objArray[ 0 ].getClass() ) ||
