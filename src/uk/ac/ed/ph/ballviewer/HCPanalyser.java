...
public class HCPanalyser {
	...
	/* Creates a new instance of the HCPanalyser. Makes space for 6 pairs of vectors. */
	public HCPanalyser() {
		vp = new VectorPair[6];
		vpindex = 0;
		vN = new Vector3();
	...
	/* Adds a new vector pair, if there is enough space. 
	 * Also keeps track of the number of attempted entries.
	 * Assumes this vector pair are involved in the characteristic 
	 * HCP angle around a central particle.
	 */
	public void addVectorPair(Vector3 v1, Vector3 v2) {
		if (vpindex<6) vp[vpindex++] = new VectorPair(v1,v2); 
		else vpindex++;  // keep track of too many entries but don't add to full array
	}
	
	/* Obtains a plane direction from the 6 vector pairs entered. */
	public Vector3 getPlaneDirection() {
		if (vpindex!=6) return new Vector3();
		
		vp[0].polarity = P;  // begin by arbitrarily setting one pair then works to set all the others
		for (int i=0;i<6;i++) if (vp[i].polarity != unknown) { 
			for (int j=0;j<6;j++) if (i!=j) {
				if 	  (vp[i].v1==vp[j].v1 || vp[i].v2==vp[j].v2) vp[j].polarity =  vp[i].polarity;
				else if (vp[i].v1==vp[j].v2 || vp[i].v2==vp[j].v1) vp[j].polarity = -vp[i].polarity;
			}
		}
		for (int i=0;i<6;i++) { // then it separates these into two sets of vectors, which get summed
			if		  (vp[i].polarity == P) { vP.add(vp[i].v1); vN.add(vp[i].v2); }
			else if (vp[i].polarity == N) { vN.add(vp[i].v1); vP.add(vp[i].v2); }
			else return new Vector3(); // ie something is wrong if neither P nor N
		}
		
		return vP.minus(vN); // the difference between the vectors is in the direction of the plane
	}
}
class VectorPair {
	public int polarity;
