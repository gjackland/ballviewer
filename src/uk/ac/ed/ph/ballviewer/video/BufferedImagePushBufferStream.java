...
public class BufferedImagePushBufferStream implements PushBufferStream, Runnable
{
	...
	public BufferedImagePushBufferStream( final BufferedImage[] images )
	{
		if( images == null || images.length == 0 || images[ 0 ].getType() != BufferedImage.TYPE_INT_RGB )
		{
		...
		rgbFormat = new RGBFormat( size, // Frame size
				...
				1, // Pixel stride (number of array elements between adjacent
					// pixels)
				size.width, // Line stride (number of array elements between
							// video lines)
				Format.FALSE, // Flipped - whether video frames are vertically
								// flipped
	...
	void start( final boolean start )
	{
		synchronized( this )
		{
