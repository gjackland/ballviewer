...
public class Vector3 extends Tuple3 implements Cloneable
{
	 ...
	 * @param r  radius
	 * @param th the polar angle (theta) in range 0.0 through <i>pi</i>
	 * @param phi the azimuthal angle in range 0.0 through 2*<i>pi</i>
	 */
	...
	public void set(Vector3 v) { this.x=v.x;this.y=v.y;this.z=v.z; }
	/** Returns the scalar (<code>double</code>) dot product of the vector with <code>v</code>. */
	...
	public double modulusSq() { return x * x + y * y + z * z ;	}
	
	/** Normalises the vector. */
	public void normalise() { this.multiply(1.0/this.modulus()); }
	/** Returns this unit vector in the direction of this vector.  */
	public Vector3 normalised() { return this.times(1.0/this.modulus()); }

	/** Returns the sum of this vector and vector v.  */
	public Vector3 plus(Vector3 v) { return new Vector3(this.x+v.x, this.y+v.y, this.z+v.z); }
	/** Returns the difference vector from <code>v</code> to this vector.  */
	public Vector3 minus(Vector3 v) { return new Vector3(this.x-v.x, this.y-v.y, this.z-v.z); }
	/** Returns the vector multiplied by scalar <code>s</code>.  */
	public Vector3 times(double s) { return new Vector3(this.x*s,this.y*s, this.z*s); }
	
	/** Adds vector <code>v</code> to this vector.  */
	public void add(Vector3 v) { this.x+=v.x; this.y+=v.y; this.z+=v.z; }
	/** Subtracts vector <code>v</code> from this vector. */
	public void subtract(Vector3 v) { this.x-=v.x; this.y-=v.y; this.z-=v.z; }
	...
	public void rotateX (double phi) { this.set(this.rotatedX(phi)); }
	/** Rotates this vector around the y axis. */
	public void rotateY (double phi) { this.set(this.rotatedY(phi)); }
	/** Rotates this vector around the z axis. */
	public void rotateZ (double phi) { this.set(this.rotatedZ(phi)); }
	
	/** 
	 * Perspectivises this vector as if the camera is at (0,0,<code>camdepth</code>). 
	 * The x,y components represent a pespective projection onto a flat screen.
	 * The z component can be used for z-buffering.
	 * At z=0.0, the scale  is 1.0 .
	 **/
	public void perspectivise(double camdepth) {
		double zsc = -camdepth/(z-camdepth); // so that zsc = 1.0 at z=0;
		this.x*=zsc; this.y*=zsc; 
	}
	/** Returns the perpective projection of this vector, as above. */
	public Vector3 perspectivised(double camdepth) {
		double zsc = -camdepth/(z-camdepth); // so that zsc = 1.0 at z=0;
		return new Vector3( this.x*zsc, this.y*zsc, this.z); //just ignore the z component
	}
	public Object clone() { return new Vector3(this.x, this.y, this.z); }
}
