...
{
	// These two lists remain in sync with one another. For each attached
	// attribute
	// exists a corresponding output map that maps the analyser output onto the
	// class type
	// of the attribute
	protected final ArrayList< SysObjAttribute >	attachedAttributes	= new ArrayList< SysObjAttribute >();
	protected final ArrayList< T >					outputMaps			= new ArrayList< T >();

	private final String							name;
	private final BallAnalyser						parentAnalyser;											// The
																												// analyser
																												// this
																												// output
																												// belong
	...
	AnalyserOutput( final String name, final BallAnalyser parentAnalyser )
	{
		this.name = name;
		this.parentAnalyser = parentAnalyser;
	}

	public final boolean isAttached( final SysObjAttribute attribute )
	{
	 ...
	 * 
	 */
	public final boolean attachAttribute( final SysObjAttribute attribute )
	{
		final Class< ? extends T > outputMapClass = getOutputMapForClass( attribute.getAttributeClass() );

		// Check that we support this attribute type and that it isn't already
		// attached
		if( outputMapClass != null && !attachedAttributes.contains( attribute ) )
		{
			// Let's try making a new instance of the output map for this type
			// of attribute
			try
			{
				...
				// Send a message to indicate that an attribute has been
				// attached
	 ...
	 * 
	 */
	public final boolean detachAttribute( final SysObjAttribute attribute )
	{
	...
	public BallAnalyser getParentAnalyser()
	{
	 ...
	 * 
	 */
	abstract Set< Class > getSupportedAttributeTypes();

	protected abstract Class< ? extends T > getOutputMapForClass( final Class sysObjClass );
