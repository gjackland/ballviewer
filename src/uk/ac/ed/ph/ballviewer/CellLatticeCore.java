...
{ 
	protected double cellSize;				// minimum size of a cell 
	protected double cx,cy,cz;				// sides of a cell = cellSize for ordinary 
	 
	protected int X, Y, Z;      		// no. of cells in each direction 
	protected ArrayList< Positionable >[][][] list;  // flexible (growable) array of Objects (including Balls and Arrows) 
	
	protected Aabb		aabb;
	
	private final SystemCell		supercell;
	 
	public CellLatticeCore(
	...
	{ 
		for( int i=1; i<=X; i++ )
		{
			for( int j=1; j<=Y; j++ )
			{
				for( int k=1; k<=Z; k++ )
				{ 
					list[i][j][k] = new ArrayList< Positionable >( 5 );   // initially makes space for 5 objects only 
				}
			}
		} 
		setLatticeEdges(); 
	...
	public LatticeCoor getLatticeCoor(
		final Vector3			v
	)
	{
	...
	/** Returns the relative positions of the 'n' nearest neighbours (in nearest 9 cells). */ 
	public Vector3[] getNearestNeighbours(
		final Ball	b,
		final int	n
	)
	{ //get positions of n nearest neighbours 
		...
		return nn; 
	} 	/** Returns the relative positions of all the neighbours within rr (radius squared) */ 
	...
	{ 
		...
		try
		{
			// the following works if the cell size (step) is greater than r (otherwise modify 1's) 
	...
	/** Returns at most 'n' of the relative positions of nbrs within rr (radius squared). */ 
	...
	{ 
		...
		for (i=0;i<v.length;i++) { tr.addNode(v[i],v[i].dot(v[i])); } // set shouldn't have v=0 
		tr.resetTraverser(); 
		// read out and return at most 'n' 
		Vector3[] nn = new Vector3[(n<v.length)? n:v.length]; 
		for (i=0;i<nn.length;i++) { tr.findNextSmallest(); nn[i] = (Vector3)tr.object(); }		 
		return nn;    
	} 
