...
class Converter
{
	...
	static int readInt( BufferedInputStream inp )
	{ // there are 10 characters for every int
		byte[] b = new byte[ 10 ];
		try
		{
	...
	static float readFloat( BufferedInputStream inp )
	{ // and 12 for every Float (including
		byte[] b = new byte[ 12 ];
		try
		{
			inp.read( b, 0, b.length );
		...
		return Float.parseFloat( new String( b ).trim() ); // spaces which are
															// removed by trim()
															// )
	}
}