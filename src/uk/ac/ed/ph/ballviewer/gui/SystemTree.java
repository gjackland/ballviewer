...
class SystemTree extends JTree implements AttributeAttachListener
{
	class AttributeHandler implements ActionListener
	{
		...
		private final HashMap< JCheckBoxMenuItem, AnalyserOutput >	menuItemMap	= new HashMap< JCheckBoxMenuItem, AnalyserOutput >();
		private AnalyserOutput										currentOutput;														// The
																																		// output
																																		// the
																																		// attribute
																																		// is
		...
		AttributeHandler( final AnalysisManager analysisManager, final SysObjAttribute attribute )
		{
		...
		public void actionPerformed( ActionEvent evt )
		{
			...
			if( output != null )
			{
				...
				{
					// Disable the check as the attribute isn't attached to an
					// output
	...
	private final HashMap< SysObjAttribute, AttributeHandler >	handlerMap	= new HashMap< SysObjAttribute, AttributeHandler >( 5 );

	public SystemTree( final AnalysisManager analysisManager )
	{
		super();
	...
	private ArrayList< SysObjAttribute > getBallAttributes()
	{
	...
	private TreePath findPathFromFromAttribute( final SysObjAttribute attribute )
	{
		// This could be improved by only searching a depth of 3 but it's not
		// terribly important as the tree is always small
	...
	public void update()
	{
		...
		model.nodeStructureChanged( rootNode ); // Tell the model that we've
												// made majour changes
		createNodes();
	}
