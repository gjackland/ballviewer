...
public class ExportWizard extends WizardDialog implements ControllerListener, DataSinkListener
{
	...
	public ExportWizard( final String strTitle, final Frame frame, final DataSource source )
	{
	...
	public ExportWizard( final Frame frame, final DataSource source )
	{
	...
	private boolean createProcessor()
	{
		...
		{
			JOptionPane.showMessageDialog( this, "Failed to connect to data source \n" + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE );
			e.printStackTrace();
			return false;
		}
		...
		{
			JOptionPane.showMessageDialog( this, JMFI18N.getResource( "jmstudio.error.processor.create" ) + "\n" + exception.getMessage(), "Error", JOptionPane.ERROR_MESSAGE );
	...
	private void setupFormatSelectPanel()
	{
	...
	@Override
	protected JPanel getNextPage( final JPanel panelPage )
	{
	...
	@Override
	protected JPanel getPrevPage( final JPanel panelPage )
	{
	...
	@Override
	protected boolean onPageActivate( final JPanel panelPage )
	{
	...
	private void configureProcessor()
	{
		...
		if( boolResult == false )
		{
			JOptionPane.showMessageDialog( frameOwner, strFailMessage, "Error", JOptionPane.ERROR_MESSAGE );
			destroyProcessor();
		}
	}
	...
	private boolean doSaveFile()
	{
		...
		if( dsOutput == null )
		{
			JOptionPane.showMessageDialog( frameOwner, JMFI18N.getResource( "jmstudio.error.processor.creatednooutput" ), "Error", JOptionPane.ERROR_MESSAGE );
		...
		{
			stopSaving();
			JOptionPane.showMessageDialog( frameOwner, exception, "Error", JOptionPane.ERROR_MESSAGE );
			return( false );
		}
		...
		try
		{
			...
			if( nMediaDuration > 0 && duration != Duration.DURATION_UNBOUNDED && duration != Duration.DURATION_UNKNOWN )
				dlgProgressSave = new ProgressDialog( frameOwner, JMFI18N.getResource( "jmstudio.saveprogress.title" ), 0, nMediaDuration, this );
			else
				dlgProgressSave = new ProgressDialog( frameOwner, JMFI18N.getResource( "jmstudio.saveprogress.title" ), JMFI18N.getResource( "jmstudio.saveprogress.label" ),
						monitor, this );
	...
	public void actionPerformed( final ActionEvent event )
	{
		...
		if( ( strCmd.equals( ProgressDialog.ACTION_ABORT ) || strCmd.equals( ProgressDialog.ACTION_STOP ) ) && dataSinkSave != null )
		{
			stopSaving();
		}
	...
	public void dataSinkUpdate( DataSinkEvent event )
	{
		...
		{
			closeDataSink();
			// MessageDialog.createInfoDialog ( frameOwner,
			// "File has been saved." );
		}
		else if( event instanceof DataSinkErrorEvent )
		{
			stopSaving();
			JOptionPane.showMessageDialog( frameOwner, JMFI18N.getResource( "jmstudio.error.processor.writefile" ), "Error", JOptionPane.ERROR_MESSAGE );
		}
	}
