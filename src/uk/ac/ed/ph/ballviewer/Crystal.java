...
/** Produces sets of coordinates of a particular crystal type. <br> The separation is 1.0 .*/
public final class Crystal {     //returns datasets of ideal crystals
	static final double cs = 1.0;	
	static int X=10,Y=10,Z=10;
	
	private Crystal() {} // This class cannot be instantiated //
	...
	static public Ball[] BCC() {
		...
		Vector3 v;
		int i;
		for (i=0;i<p.length;i++) p[i] = new Ball(Color.blue);
		i=0;
	...
	static public Ball[] HCP() {
		...
		Vector3 v;
		int i;
		for (i=0;i<p.length;i++) p[i] = new Ball(Color.red);
		i=0;
		for (int ix=0;ix<X;ix++)for (int iy=0;iy<Y;iy++)for (int iz=0;iz<Z;iz++) {
	...
	/** Returns a set of 13 Balls arranged in icosahedral formation around central Ball. */
	static public Ball[] ICO() {
		double s = 3*cs;
		final double theta = (double)1.10714871779409;   // cos(th)= cos(pi/5)/(cos(pi/5)+1)
		Ball[] p = new Ball[13];
		double phi;
		int i;
		Vector3 v;
